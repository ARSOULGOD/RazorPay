...
export type DiscrepancyTaxonomyId =
  | "exactMatch"
  | "settlementLag"
  | "feeDeduction"
  | "partialCapture"
...
export const DISCREPANCY_TAXONOMY: readonly DiscrepancyTaxonomyEntry[] = [
  {
    id: "exactMatch",
    label: "Exact match",
    description:
      "Baseline — same ID/amount/date across all 3 sources.",
  ...
  },
  {
    ...
    description:
      "Bank shows T+2, ledger shows T+0 (date mismatch between bank and ledger).",
    affectedTables: ["BankTransaction", "LedgerEntry", "SettlementRecord"],
    isOrphan: false,
  },
  {
    ...
    label: "Fee deduction",
    description:
      "Settlement net = gross − fee − tax; ledger shows gross (amount mismatch).",
    affectedTables: ["SettlementRecord", "LedgerEntry", "BankTransaction"],
    isOrphan: false,
  },
  {
    id: "partialCapture",
    ...
    description:
      "Only part of an amount is captured or refunded (ledger entryType / amount mismatch).",
  ...
  },
  {
    ...
    label: "Split payment",
    description:
      "One ledger entry corresponds to two bank transactions (ledger:bank = 1:2).",
  ...
  },
  {
    ...
    label: "Many-to-one",
    description:
      "Two or more ledger entries correspond to one bank transaction (batched payout).",
    affectedTables: ["LedgerEntry", "BankTransaction", "SettlementRecord"],
    isOrphan: false,
  },
  {
    ...
    description:
      "Same transaction logged twice in one source (data-entry-error simulation).",
    affectedTables: ["BankTransaction", "LedgerEntry", "SettlementRecord"],
    isOrphan: false,
  },
  {
    ...
    description:
      "Small paise-level rounding difference in amount across sources.",
    affectedTables: ["BankTransaction", "LedgerEntry", "SettlementRecord"],
    isOrphan: false,
  },
  ...
  // chosen per-instance by the generator, never all three.
  {
    ...
    label: "True orphan",
    description:
      "Exists in exactly one source and matches nothing in the others.",
    affectedTables: ["BankTransaction", "LedgerEntry", "SettlementRecord"],
    isOrphan: true,
  },
];
...
export const DISCREPANCY_TAXONOMY_BY_ID: Record<
  DiscrepancyTaxonomyId,
  DiscrepancyTaxonomyEntry
> = Object.fromEntries(
  DISCREPANCY_TAXONOMY.map((entry) => [entry.id, entry]),
) as Record<DiscrepancyTaxonomyId, DiscrepancyTaxonomyEntry>;
