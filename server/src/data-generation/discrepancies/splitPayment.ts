 ...
 */
export function generateSplitPayment(
  event: TrueEvent,
  indexSeed: number,
  ...
  if (part1Paise <= 0 || part2Paise <= 0) {
    throw new Error(
      `generateSplitPayment: non-positive split for ${event.eventId} ` +
        `(total=${event.amount}, pct=${pct})`,
    );
  }
  if (part1Paise + part2Paise !== totalPaise) {
    throw new Error(
      `generateSplitPayment: paise sum invariant broken for ${event.eventId}`,
    );
  }

  const ledgerAmount = paiseToAmount(totalPaise);
  const part1Amount = paiseToAmount(part1Paise);
  const part2Amount = paiseToAmount(part2Paise);
  ...
  return {
    ...
    settlementRows: [],
    discrepancyType: TAXONOMY_IDS.splitPayment,
  };
}
