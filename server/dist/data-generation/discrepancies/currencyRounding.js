...
exports.generateCurrencyRounding = generateCurrencyRounding;
 ...
 */
function generateCurrencyRounding(event, indexSeed) {
    const ids = (0, types_1.deterministicIds)(event.eventId, indexSeed);
    const exactPaise = (0, types_1.amountToPaise)(event.amount);
    ...
    if (roundedPaise <= 0) {
        throw new Error(`generateCurrencyRounding: non-positive rounded amount for ${event.eventId} ` +
            `(exact=${event.amount}, delta=${signedDelta})`);
    }
    const exactAmount = (0, types_1.paiseToAmount)(exactPaise);
    const roundedAmount = (0, types_1.paiseToAmount)(roundedPaise);
    ...
    return {
        ...
        discrepancyType: "currencyRounding",
    };
}
//# sourceMappingURL=currencyRounding.js.map